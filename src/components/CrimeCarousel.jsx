...
function CrimeCarousel(props) {
    ...
    const fetchCarouselItems = async () => {
        ...
        let randPage =  Math.floor(Math.random() * (106 - 1 + 1)) + 1;
        let randPageStr = randPage.toString();
        
        const tvListResponse = await fetch(`https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${randPageStr}&sort_by=popularity.desc&with_genres=80&with_original_language=en`, options);
